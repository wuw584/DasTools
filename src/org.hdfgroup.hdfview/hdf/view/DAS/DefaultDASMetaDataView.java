...
*/
public class DefaultDASMetaDataView implements MetaDataView {

    private static final Logger log = LoggerFactory.getLogger(DefaultDASMetaDataView.class);
    ...
    protected final Composite             PreviewPane;

    /** The general metadata pane */
    protected final Composite             generalObjectInfoPane;

    protected final Composite             DASObjectPane;
    ...
    private static final int              Preview_TAB_INDEX = 0;
    private static final int              GENERAL_TAB_INDEX = 1;

    private static final int              DAS_TAB_INDEX = 2;
    ...
    */
    public DefaultDASMetaDataView(Composite parentComposite, DataViewManager viewer, HObject theObj) {
        log.trace("generate das view window");
        ...
        contentTabFolder.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                switch (contentTabFolder.getSelectionIndex()) {
                    case Preview_TAB_INDEX:
                        parent.setData("MetaDataView.LastTabIndex", Preview_TAB_INDEX);
                        break;
                    case GENERAL_TAB_INDEX:
                        parent.setData("MetaDataView.LastTabIndex", GENERAL_TAB_INDEX);
                        break;
                    case DAS_TAB_INDEX:
                        parent.setData("MetaDataView.LastTabIndex", DAS_TAB_INDEX);
                        break;
                    default:
                        parent.setData("MetaDataView.LastTabIndex", GENERAL_TAB_INDEX);
                        break;
        ...
        PreviewPane = createAttributeInfoPane(contentTabFolder, dataObject);
        if (PreviewPane != null) {
            TabItem PreviewItem = new TabItem(contentTabFolder, SWT.None, Preview_TAB_INDEX);
            PreviewItem.setText("Preview");
            PreviewItem.setControl(PreviewPane);
        }
        ...
        DASObjectPane  = createDASObjectInfoPane(contentTabFolder, viewer.getTreeView().getTree());
        if(DASObjectPane != null){
            TabItem DASItem = new TabItem(contentTabFolder, SWT.None, DAS_TAB_INDEX );
            DASItem.setText("DAS Object Info");
            DASItem.setControl(DASObjectPane);
        }
    ...
    private Composite createDASObjectInfoPane(Composite parent, final Tree dataTree) {
        if (parent == null || dataObject == null) return null;
        ...
        attrNumberLabel = new Label(attributeInfoGroup, SWT.RIGHT);
        attrNumberLabel.setFont(curFont);
        attrNumberLabel.setText("Number of event = 0");
        attrNumberLabel.setLayoutData(new GridData(SWT.BEGINNING, SWT.FILL, false, false));

        Button addButton = new Button(attributeInfoGroup, SWT.PUSH);
        addButton.setFont(curFont);
        addButton.setText("choose event list");
        addButton.setLayoutData(new GridData(SWT.END, SWT.FILL, true, false));
        addButton.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                openLocalEventFile(parent);
                text.setText(eventFile.getAbsolutePath());
            }
        });
 ...
 // use to read a .txt file to analysis DAS dir
    private void openLocalEventFile(Composite parent) {
        log.trace("openLocalEventFile");
        FileDialog fChooser = new FileDialog(Display.getDefault().getShells()[0], SWT.OPEN | SWT.MULTI);
        String modeStr = "Read-only";
        ...
        String[] selectedFilenames = fChooser.getFileNames();
        if(selectedFilenames.length <= 0)
            return;

        eventFile = new File(fChooser.getFilterPath() + File.separator + selectedFilenames[0]);
