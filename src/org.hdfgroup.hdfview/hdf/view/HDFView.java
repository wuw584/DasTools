...
public class HDFView implements DataViewManager
{
    ...
    private String dasDir = null;

    private static boolean DASenable = false;
    ...
    private void createToolbar(final Shell shell) {
        ...
        //adding DAS feature
        new ToolItem(toolBar, SWT.SEPARATOR).setWidth(4);
        ToolItem startTimeItem = new ToolItem(toolBar, SWT.PUSH);
        // hdf5Item.setImage(ViewProperties.getH5Icon());
        //        timeItem.setBackground(Color.RED);
        startTimeItem.setToolTipText("time select");
        startTimeItem.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
               Dialog_Calendar dialog=new Dialog_Calendar(shell,SWT.DIALOG_TRIM|SWT.APPLICATION_MODAL);
               dialog.open();
               startTimeItem.setText(selectedTime);

            }
        });
        startTimeItem.setText("start time");

        new ToolItem(toolBar, SWT.SEPARATOR).setWidth(4);

        ToolItem endTimeItem = new ToolItem(toolBar, SWT.PUSH);
        // hdf5Item.setImage(ViewProperties.getH5Icon());
        //        timeItem.setBackground(Color.RED);
        endTimeItem.setToolTipText("time select");
        endTimeItem.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
               Dialog_Calendar dialog=new Dialog_Calendar(shell,SWT.DIALOG_TRIM|SWT.APPLICATION_MODAL);
               dialog.open();
               endTimeItem.setText(selectedTime);

            }
        });
        endTimeItem.setText("end time");
    ...
    //add DAS method
    //https://blog.csdn.net/weixin_44020515/article/details/105063784

    public static void setSelectedTime(String str){
        System.out.println(str);    
        selectedTime = "s"+str;
    }

    public boolean getDASenable(){
        return DASenable;
    }
        ...
        //                        +hours+":"+minutes+":"+time.getSeconds();
            ...
            btnOk.addSelectionListener(new SelectionAdapter() {
                ...
                public void widgetSelected(SelectionEvent e) {                
                    ...
                    //将selectDate字符串传到starttime 中
                    setSelectedTime(selectedDate);
                }
            });
            btnOk.setBounds(510, 249, 68, 34);
            btnOk.setText("OK");
        }
    }

    private void openLocalDir(String dirname){
        DASenable = true;
        log.trace("openLocalDir {}",dirname);

        if(dirname != null){
            DirectoryDialog dChooser = new DirectoryDialog(mainWindow , SWT.OPEN | SWT.MULTI);
            dChooser.setText(mainWindow.getText() + " - Open Dir ");
            dChooser.setFilterPath(currentDir);
            dasDir = dChooser.open();
            urlBar.add(dasDir,1);
            urlBar.select(1);
            //TODO：加上多选目录

            //显示第一个file的相关信息，使用treeview 第一个文件
            File dir = new File(currentDir);
            File[] files = dir.listFiles();
            log.trace("DAS dir exmple file pwd:{}",files[0].getPath());
            ...
            //显示第一个file的相关信息，使用treeview 第一个文件
            File dir = new File(dasDir);
            File[] files = dir.listFiles();
            String firstfile = files[0].getPath();
            // log.trace("DAS dir{} exmple file pwd:{}",dir,firstfile);
            int accessMode = FileFormat.READ;
            
            for(int i = 0; i < files.length; i++){
                log.trace("openLocalFile treeView.openFile {}",firstfile);
                        try {
                            treeView.openFile(files[i].getPath(), accessMode + FileFormat.OPEN_NEW);
                        }
                        catch (Exception ex) {
                            try {
                                treeView.openFile(firstfile, FileFormat.READ);
                            }
                            catch (Exception ex2) {
                                display.beep();
                                urlBar.deselectAll();
                                Tools.showError(mainWindow, "Open", "Failed to open file " + files[0] + "\n" + ex2);
                                currentFile = null;
                            }
                        }
            }
